...
async function checkUserNotes() {
  try {
    ...
    // Проверим заметки пользователя (в таблице notes колонка называется title)
    const notesResult = await pool.query(
      'SELECT id, title, folder_id, created_at FROM notes WHERE user_id = $1 ORDER BY created_at',
      [user.id]
    );
    
    console.log('\nЗаметки пользователя:');
    notesResult.rows.forEach(note => {
      console.log('- ' + note.title + ' (ID: ' + note.id + ', Folder: ' + (note.folder_id || 'root') + ')');
    });
